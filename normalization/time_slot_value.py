    ...
    def parse(self):
        if self.timelist.type == 'time_point':
            ...
            if not commonParser.timeUnit[2]:
                ...
                month_proc.get_month(self.timelist.entity, self.basetime, commonParser)
            if not commonParser.timeUnit[3] and commonParser.timeFormatInfo == "":
                day_proc = Day()
                day_proc.get_day(self.timelist.entity, self.basetime, commonParser)
            if not commonParser.timeUnit[3] and commonParser.timeFormatInfo == "":
                day_proc = lunarDay()
                day_proc.get_day(self.timelist.entity, self.basetime, commonParser)
            if not commonParser.timeUnit[4] and commonParser.timeFormatInfo == "":
                apm_proc = Apm()
                apm_proc.get_apm_info(self.timelist.entity, commonParser)
            if not commonParser.timeUnit[5] and commonParser.timeFormatInfo == "":
                hour_proc = Hour()
                hour_proc.get_hour(self.timelist.entity, self.basetime, commonParser)
            if not commonParser.timeUnit[6] and commonParser.timeFormatInfo == "":
                minute_proc = Minute()
                minute_proc.get_minute(self.timelist.entity, self.basetime, commonParser)
            if not commonParser.timeUnit[7] and commonParser.timeFormatInfo == "":
                second_proc = Second()
